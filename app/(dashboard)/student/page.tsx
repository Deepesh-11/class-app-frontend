...
export default function StudentHomePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [classroomRes, coursesRes, attendanceRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/student/me/classroom`),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/student/me/courses`),
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/student/courses" className="text-xs text-gray-500 hover:text-gray-900 transition">
              View all →
            </Link>
          </div>
          {courses.length === 0 ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/student/attendance" className="text-xs text-gray-500 hover:text-gray-900 transition">
              View all →
            </Link>
          </div>
          {attendance.length === 0 ? (
