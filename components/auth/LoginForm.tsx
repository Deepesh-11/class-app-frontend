...
export default function LoginForm() {
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true)
    try {
      await verifyOtp(formData.email, otp) // sets cookie internally
      ...
      {step === "login" ? (
        <>
          ...
          <div className="text-right mb-5">
            <Link href="#" className="text-xs text-gray-500 hover:text-gray-800">Forgot password?</Link>
          </div>
        ...
        <>
          ...
          <p className="text-sm text-gray-500 mb-7">
            We sent a one-time code to <span className="text-gray-800 font-medium">{formData.email}</span>.
          </p>
          ...
          <div className="mb-5">
            ...
            <input
              ...
              className="w-full h-10 px-3 text-sm border border-gray-200 rounded-lg outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-100 transition tracking-widest"
            />
          </div>
